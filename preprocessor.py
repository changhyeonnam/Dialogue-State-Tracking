...
class TRADEPreprocessor(DSTPreprocessor):
    ...
    def _convert_example_to_feature(self, example):
        ...
        # if args.mecab == "True":
        #     context_turn = list(map(' '.join,map(mecab.morphs,example.context_turns)))
        #     current_turn = list(map(' '.join,map(mecab.morphs,example.current_turn)))
        #     dialogue_context = " [SEP] ".join(context_turn+current_turn)
        # else:
        dialogue_context = " [SEP] ".join(example.context_turns + example.current_turn)

        input_id = self.src_tokenizer.encode(dialogue_context,truncation=True,max_length=512,add_special_tokens=False)
        ...
        for slot in self.slot_meta:
            ...
            # if args.mecab == 'True':
            #     mecab = Mecab()
            #     value = mecab.morphs(value)
            #     value = ' '.join(value)
            #     target_id = self.trg_tokenizer.encode(value, add_special_tokens=False) + [self.trg_tokenizer.sep_token_id]
            # else:
            target_id = self.trg_tokenizer.encode(value, add_special_tokens=False) + [self.trg_tokenizer.sep_token_id]
    ...
    def collate_fn(self, batch):
        ...
        segment_ids = torch.LongTensor(
            self.pad_ids([b.segment_id for b in batch], 0)
        )
